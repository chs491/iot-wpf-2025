...
PUB_ID = 'IOT75' # 본인 아이피 마지막주소
BROKER = '210.119.12.75' # 본인 아이피
PORT = 1883
...
try:
    ...
    while True:
        ...
        COUNT += 1
        ## 센싱데이터를 json형태로 변경
        ## OrderedDict 로 먼저 구성. 순서가 있는 딕셔너리타입 객체
        raw_data = OrderedDict()
        raw_data['PUB-ID'] = PUB_ID
        raw_data['COUNT'] = COUNT
        raw_data['SENSING_DT'] = currtime.strftime('%Y-%m-%d %H:%M:%S') # 'yyyy-MM-dd HH:mm:ss'
        raw_data['TEMP'] = f'{temp:0.1f}'
        raw_data['HUMID'] = f'{humid:0.1f}'
        raw_data['LIGHT'] = photo if photo >= 200 else 0
